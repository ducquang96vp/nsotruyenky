       ...
       boolean x99 ;
        if (rc != null) {
            for (int i = 0; i < number; i++) {
                ...
                int itemId = rc.next();
                Item itm = ItemFactory.getInstance().newItem(itemId);
                int[] arrItem = {1095, 1096, 1097, 1098, 1099, 1100, 1101, 1102, 1102, 1103, 1104, 1105, 1106, 1107,
                    1108, 1130, 1129, 1131, 1132, 1133, 1134};
                 x99 = Arrays.stream(arrItem).anyMatch(ii -> ii == itemId);
                if(x99){
                     itm = ItemFactory.getInstance().newItem9X(itemId, NinjaUtils.nextInt(21) == 1);
                }    
                itm.initExpire();
                if (itm.id == ItemName.THONG_LINH_THAO) {
                    itm.setQuantity(NinjaUtils.nextInt(5, 10));
