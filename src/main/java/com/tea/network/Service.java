...
public class Service extends AbsService {
   ...
   public void NhanthuongCT(String text, boolean reward) {
        try {

            Message ms = messageNotMap(CMD.REWARD_CT);
            DataOutputStream ds = ms.writer();
            ds.writeUTF(text);
            ds.writeBoolean(reward);
            ds.flush();
    ...
    public void viewInfo(Char pl) {
        try {
            ...
            ds.writeShort(321);// diem hoat dong  Duydebug
            if (pl.clan == null) {
                ds.writeUTF("");
            } else {
